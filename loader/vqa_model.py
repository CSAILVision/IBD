...
class SimpleVQANet(nn.Module):
    def __init__(self, embedding_tokens):
        ...
        concat_feature = 1024
        vision_features = settings.OUTPUT_FEATURE_SIZE

        self.fc_q = nn.Linear(question_features, concat_feature)
        self.pool_v = nn.AvgPool2d(7)
        self.fc_v = nn.Linear(vision_features, concat_feature)
        ...
        self.classifier = nn.Linear(concat_feature, settings.MAX_ANSWERS)

    def forward(self, v, q, q_len):
        q = self.text(q, list(q_len.data))
        q = self.fc_q(q)

        v = self.pool_v(v).squeeze()
        if len(v.shape) == 1:
            v = v.unsqueeze(0)
...
class VQANet(nn.Module):
    ...
    def __init__(self, embedding_tokens):
        ...
        self.classifier = Classifier(
            ...
            mid_features=1024,
            out_features=settings.MAX_ANSWERS,
            drop=0.5,
        )

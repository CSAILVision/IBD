...
class CaptionGenerator(object):
    ...
    def beam_search(self, rnn_input, initial_state=None):
        ...
        """
        batch_size = rnn_input.data.shape[1]
        word_vars = []
        ...
        for _ in range(self.max_caption_length - 1):
            ...
            input_feed = Variable(partial_captions_list[0].sentence[-1], volatile=True)
            ...
            for i, partial_caption in enumerate(partial_captions_list):
                if isinstance(new_states, tuple):
                    state = (new_states[0].narrow(1, i, batch_size),
                             new_states[1].narrow(1, i, batch_size))
                else:
                    state = new_states[i]
                for k in range(self.beam_size):
                    w = words[:, k]
                    sentence = partial_caption.sentence + [w]
                    logprob = partial_caption.logprob + logprobs[:, k]
                    score = logprob
        ...
        return [[ list(sent) for sent in c.sentence ]for c in caps], [c.score for c in caps], word_vars
